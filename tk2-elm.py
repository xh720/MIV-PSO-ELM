...
text.pack(fill=tk.X, pady=10, side=tk.BOTTOM)
...
data_table.pack(anchor='nw', expand=True, fill="both",
                ipadx=40, ipady=10, padx=20, pady=10, side='left')


var_frame1 = tk.Frame(window)
var_frame1.pack(fill=tk.BOTH, side='top')

var_frame2 = tk.Frame(window)
var_frame2.pack(fill=tk.BOTH, side='top')

var_frame3 = tk.Frame(window)
var_frame3.pack(fill=tk.BOTH, side='top')

# 创建显示结果的文本框
result_widget = scrolledtext.ScrolledText(window, height=25, width=28)
result_widget.pack(fill=tk.Y, padx=5, pady=5, side='left')
...
# hi_lable.place(x=530, y=25)

var_hidden = tk.StringVar()
var_hidden.set('50')
entry_hidden = tk.Entry(var_frame1, textvariable=var_hidden, width=12)
entry_hidden.pack(pady=2.5, side="left")
# entry_hidden.place(x=620, y=25)


activationFunc_lable = tk.Label(var_frame2, text='激活函数：')
activationFunc_lable.pack(pady=2.5, side="left")
# activationFunc_lable.place(x=530, y=55)
...
# entry_activationFunc.place(x=620, y=55)

type__lable = tk.Label(var_frame3, text='类型：')
type__lable.pack(pady=2.5, side="left")
# type__lable.place(x=530, y=85)

var_type_ = tk.StringVar()
var_type_.set('REGRESSION')
entry_type_ = tk.Entry(var_frame3, textvariable=var_type_, width=12)
# entry_type_.place(x=620, y=85)
entry_type_.pack(padx=47, pady=5, side="left")


# 导入数据按钮
btn_import = tk.Button(button_frame, text='导入数据', width=12, height=2,
                       command=import_data)
btn_import.pack(side="left", expand=True)
# btn_import.place(x=10, y=320)

# 数据标准化按钮
btn_std = tk.Button(button_frame, text='标准化数据', width=12, height=2,
                      command=standardize_data)
btn_std.pack(side="left", expand=True)
# btn_std.place(x=130, y=320)

# 划分数据集按钮
btn_split = tk.Button(button_frame, text='划分数据集', width=12, height=2,
                      command=split_data)
btn_split.pack(side="left", expand=True)
# btn_split.place(x=250, y=320)

# 神经网络初始化按钮
btn_train = tk.Button(button_frame, text='构建网络', width=12, height=2,
                      command=crate_ELM)
btn_train.pack(side="left", expand=True)
# btn_train.place(x=370, y=320)

# 训练数据集按钮
btn_test = tk.Button(button_frame, text='训练数据集', width=12, height=2,
                     command=beg_train)
btn_test.pack(side="left", expand=True)
# btn_test.place(x=490, y=320)

# 测试数据集按钮
btn_predict = tk.Button(button_frame, text='测试数据集', width=12, height=2,
                     command=beg_test)  # 点击按钮式执行的命令
btn_predict.pack(side="left", expand=True)
# btn_predict.place(x=610, y=320)

# 测试数据集按钮
btn_predict = tk.Button(button_frame, text='图像展示', width=12, height=2,
                     command=show_prediction_comparison)  # 点击按钮式执行的命令
btn_predict.pack(side="left", expand=True)
